...
parser_POS.add_argument('--var2location',
                        help='Path to the annotation file generated by the var2location.pl script (ENSEMBL SQL connection)',
                        metavar="VAR2LOC_FILE", required=False)
        ...
        for ic in tqdm(range(0, hm_chunks)):
            ...
            try:
                ...
                if ic == 0:
                    ...
                    add_header = False
                df_chunk.to_csv(hm_out, mode='a', index=False, header=add_header, sep='\t')  # Write output using pandas
            except:
                hm_Passed = False
        hm_Passed = 'COMPLETED'
    ...
    if pd.isnull(v['hm_source']) is False:
        ...
        if other_allele is None:
            if returnOtherAllele is True:
                other_allele, hm_TF, hm_vid, hm_code = v_records.infer_OtherAllele(eff=effect_allele, oa_ensembl=hm_inferOtherAllele)
                hm_matchesVCF, hm_isPalindromic, hm_isFlipped = hm_TF
            else:
                hm_TF, hm_vid = v_records.check_alleles(eff=effect_allele)
                hm_matchesVCF, hm_isPalindromic, hm_isFlipped = hm_TF
        else:
            hm_TF, hm_vid = v_records.check_alleles(eff=effect_allele,
                                                    oa=other_allele)
...
def run_HmVCF(args):
    ...
    # Initiate the progress bar on dataframe
    tqdm.pandas()
    ...
    hm_cols = ['hm_source', 'hm_vid', 'hm_code']
    return_other_allele=False
    if args.addOtherAllele is True:
        hm_cols.append('other_allele')
        return_other_allele=True
    ...
    while hm_Passed is True:
        try:
            ...
            for hm_chr, df_chrom in df_scoring.groupby('hm_chr'):
                ...
                df_chrom[hm_cols] = df_chrom.progress_apply(variant_HmVCF,
                                                            axis=1,
                                                            vcfs_targetbuild=vcfs_targetbuild,
                                                            CohortVCF=usingCohortVCF,
                                                            returnOtherAllele=return_other_allele)
                ...
                if chrcount == 0:
                    ...
                    add_header = False
                df_chrom.to_csv(hm_out, mode='a', index=False, sep='\t', header=add_header)  # Write output using pandas
                chrcount += 1
            hm_Passed = 'COMPLETED'
        except:
            hm_Passed = False
